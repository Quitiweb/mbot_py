...
class MBotController:
    ...
    def _do_movement(self, movement_type):
        """Ejecuta movimientos específicos con verificación de parada"""
        if not self.mbot:
            return
        ...
        try:
            if movement_type == "bounce":
                ...
                for _ in range(3):
                    if not self.is_performing_gesture:  # Verificar si debe parar
                        break
                    self.mbot.doMove(150, 150)
                    time.sleep(0.2)
                    if not self.is_performing_gesture:
                        break
                    self.mbot.doMove(-100, -100)
                    time.sleep(0.15)
                self.mbot.doMove(0, 0)
                ...
                # Giro completo
                self.mbot.doMove(100, -100)
                for _ in range(20):  # Dividir en pasos pequeños
                    if not self.is_performing_gesture:
                        break
                    time.sleep(0.1)
                self.mbot.doMove(0, 0)
                ...
                for _ in range(15):  # Dividir en pasos
                    if not self.is_performing_gesture:
                        break
                    time.sleep(0.1)
                self.mbot.doMove(0, 0)
    ...
    def stop_gesture(self):
        ...
        if self.gesture_thread and self.gesture_thread.is_alive():
            self.gesture_thread.join(timeout=1.0)  # Esperar máximo 1 segundo
            if self.gesture_thread.is_alive():
                print("⚠️  Thread de gesto no terminó rápidamente")
